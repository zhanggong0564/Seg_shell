...
DetTrt::DetTrt(input_init inputs) :trt_file(inputs.model_path){
    assert(inputs.NumClass==4);
    NumClass = inputs.NumClass;
    ...
    this->OUTPUT_SIZE = inputs.INPUT_W*inputs.INPUT_H*inputs.NumClass;
    if(!trt_file) { //打开失败
...
void DetTrt::PostProcess(float* prob, cv::Mat &mask) {
    ...
    int i,j;
    std::array<float, NUMCLASS> results_extra{};//3:类别数目
    int result_[m_height * m_width];//拉平图像
    //每次取出4个值，
    for (i = 0,j=0;i <OUTPUT_SIZE&&j<(m_height*m_width) ; i+=NUMCLASS,j++) {
        results_extra[0] = prob[i];
        results_extra[1] = prob[i + 1];
        results_extra[2] = prob[i + 2];
        results_extra[3] = prob[i + 3];
